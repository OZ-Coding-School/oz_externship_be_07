...
from apps.questions.views.admin.admin_qna_questions_detail_delete_view import (
    AdminQuestionDetailDeleteCombinedView,
)
...
urlpatterns = [
    # 1. 카테고리 목록 조회 및 생성
    path("admin/qna/categories/", AdminCategoryListAPIView.as_view(), name="admin-category"),
    # 2. 특정 카테고리 삭제
    path("admin/qna/categories/<int:category_id>/", AdminCategoryDeleteAPIView.as_view(), name="admin-category-delete"),
    # 3. 질의 응답 목록 조회
    path("admin/qna/questions/", AdminQuestionListAPIView.as_view(), name="admin-question-list"),
    # 4. 질의 응답 상세 조회 및 삭제 (통합 뷰 연결)
    path(
    ...
    # 5. 답변 삭제
    path("admin/qna/answers/<int:answer_id>/", AdminAnswerDeleteAPIView.as_view(), name="admin-answer-delete"),
]
