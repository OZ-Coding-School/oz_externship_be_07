...
class StudentEnrollmentRequestService:
    ...
    def accept_enrollments(*, enrollment_ids: list[int]) -> None:
        enrollments = EnrollmentRequest.objects.select_for_update().filter(id__in=enrollment_ids)
        ...
        if not_found_ids:
            raise ValidationError(
                detail={"enrollments": [f"존재하지 않는 수강생 등록 요청 ID가 포함되어 있습니다: {not_found_ids}"]}
            )

        not_pending_ids = sorted(
            enrollments.exclude(status=StudentEnrollmentRequestsStatus.PENDING).values_list("id", flat=True)
        )
        if not_pending_ids:
            raise ValidationError(
                detail={
                    "enrollments": [f"승인 가능한 상태(PENDING)가 아닌 요청이 포함되어 있습니다: {not_pending_ids}"]
                }
            )
